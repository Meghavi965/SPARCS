...
class SpectralDisentangledEncoder(nn.Module):
    """
    DeBERTa-v3 encoder that outputs continuous pooled embeddings (E_P)
    ...
    def forward(self, input_ids: torch.Tensor, attention_mask: torch.Tensor) -> Tuple[torch.Tensor, torch.Tensor]:
        ...
        # Align precision between encoder outputs and spectral-normalized classifier weights
        logits = self.classifier(E_P.to(dtype=self.classifier.weight.dtype))
        return E_P, logits
